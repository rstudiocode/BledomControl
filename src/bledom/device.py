...
class BleLedDevice:
    ...
    async def set_color(self, r: int | str, g: int = None, b: int = None):
        """
        Show a solid colour.

        The values for `red`, `green` and `blue` are expected to be integers
        with values between 0 and 255 (inclusive).
        Or hex color in one str parameter.

        :param r: int red channel or str hex color
        ...
        if isinstance(r, str):
            rgb = tuple(int(r.lstrip('#')[i:i + 2], 16) for i in (0, 2, 4))
            await self.generic_command(0x05, 0x03, *rgb)
        elif isinstance(r, int) and isinstance(g, int) and isinstance(b, int):
            await self.generic_command(0x05, 0x03, r, g, b)
        else:
